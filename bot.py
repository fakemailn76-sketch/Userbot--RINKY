...
SESSION_STRING = "1BVtsOGYBuyqnF_-otZspew7PPSxmN725Q74MIO4zzEs3IkkWcaTaJijsY4mYM_S5icr0KyidxtAfWnT42aMvalLivBA_2nv2RhGv6SQ-2i6gXrsLLLspWd-_uE7mNsugE_pQenjNpMnnSBR9bO5EhrLHdKTtqjGOOXoBBmGvNOVbK4HHqYUiKopbfEIu6YWJ455q6TnUD5K0eu_RXrwdE9QS89EbSEFbFHAsaPn-57U0kp0EkISMAwnRcM6rbn3RWOs6O6WHHcH2YqEhAIYdH4XhKaCOZ9KXDg70OhlipLaLaTXLRlS9EqkfG3VJIoWvOs8HV3O4D_d6B8cbHghIC8Ld8mGjfnY="
ADMIN_ID = 7938724671
...
STATE_FILE = f"/app/data/{BOT_NAME}_scheduler.json"

GITHUB_TOKEN = "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"
GIST_ID = "6527d9e1b11290190d513faee2e22306"

GIST_FILENAME = f"{BOT_NAME}_scheduler.json"
...
async def save_scheduler_state():
    ...
    try:
        ...
        logging.error(
            f"❌ Failed to save scheduler state: {e}"
        )


async def upload_state_to_gist():
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.patch(
                ...
                if resp.status==200:
                    logging.info("☁️ Gist Updated")

                else:
                    logging.warning(f"Gist Upload Failed {resp.status}")
...
async def download_state_from_gist():
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                ...
                if resp.status != 200:
                    logging.warning(
                        f"Gist Download Failed {resp.status}"
                    )
                    return
        ...
        if GIST_FILENAME not in data.get("files", {}):

            logging.info(
                f"📄 {GIST_FILENAME} not found in Gist. "
                "A new file will be created on first save."
            )
        ...
        logging.info("☁️ Gist Loaded")

    except Exception as e:

        logging.error(
            f"❌ Gist Download Error: {e}"
        )
...
async def load_scheduler_state():
    ...
    global current_message, send_interval, GROUP_IDS
    ...
    try:
        ...
        logging.info(
            f"♻️ Scheduler state restored ({len(GROUP_IDS)} groups)"
        )
...
async def handle_start(event):
    ...
    await event.reply(
        ...
        "/update all - সব running group এর message update করুন\n"
        "/update <group_id> - নির্দিষ্ট group এর message update করুন\n"
        "/resume&update all - resume + update সব running group\n"
        "/resume&update <group_id> - resume + update single group\n"
...
async def handle_resume(event):
    ...
    await download_state_from_gist()
...
# -----------------------------
# Update running schedule message
# -----------------------------
@client.on(events.NewMessage(pattern=r'^/update(?:\s+(all|\-?\d+))?$'))
async def handle_update(event):
    ...
    if not event.is_reply:
        ...
        await event.reply(
            "⚠️ নতুন message reply দিয়ে command দিন",
            parse_mode="md"
        )
    ...
    if not arg or arg.lower() == "all":
        ...
        for gid in GROUP_IDS:
            ...
            # override msg update
            if gid in group_overrides:
                group_overrides[gid]["message"] = new_message

        await save_scheduler_state()

        await event.reply(
            "✅ সব running group এর message update করা হয়েছে\n"
            "⏰ আগের exact timing unchanged থাকবে"
        )
    ...
    try:
        ...
        await event.reply(
            f"✅ Message update করা হয়েছে → {gname} ({gid})\n"
            f"⏰ আগের exact timing unchanged থাকবে"
        )
...
# Resume + Update
# -----------------------------
@client.on(events.NewMessage(pattern=r'^/resume&update(?:\s+(all|\-?\d+))?$'))
async def handle_resume_update(event):
    ...
    if not arg or arg.lower() == "all":
        ...
        await event.reply(
            "✅ সব running schedule resume + update করা হয়েছে\n"
            "⏰ Exact পুরানো timing maintain থাকবে"
        )

        return

    # -----------------------------
    # Resume + Update single group
    # -----------------------------
    try:
        ...
        await event.reply(
            f"✅ Resume + Update করা হয়েছে → {gname} ({gid})\n"
            f"⏰ Exact পুরানো timing maintain থাকবে"
        )
